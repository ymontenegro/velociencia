...
const CONTENT_DIR = path.join(process.cwd(), "content", "competencia");
...
export default async function CompetenciaArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const filePath = path.join(CONTENT_DIR, `${slug}.md`);
  ...
  const relatedArticles = getAllArticles("competencia")
  ...
  return (
    <article>
      ...
      <div className="mx-auto max-w-[68ch] px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div className="prose prose-lg prose-competencia">
          {mdxContent}
        </div>
      </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {source.type && (
                      <span
                        ...
                        style={{
                          backgroundColor: `var(--color-competencia-light)`,
                          color: `var(--color-competencia-dark)`,
                        }}
                      >
                        {source.type}
                      </span>
