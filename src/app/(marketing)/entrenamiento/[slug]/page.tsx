...
const CONTENT_DIR = path.join(process.cwd(), "content", "entrenamiento");
...
export default async function EntrenamientoArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const filePath = path.join(CONTENT_DIR, `${slug}.md`);
  ...
  const relatedArticles = getAllArticles("entrenamiento")
  ...
  return (
    <article>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, var(--color-entrenamiento-dark) 0%, var(--color-entrenamiento) 50%, var(--color-entrenamiento-light) 100%)`,
          minHeight: "40vh",
        }}
      >
      ...
      <div className="mx-auto max-w-[68ch] px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div className={`prose prose-lg prose-entrenamiento`}>
          {mdxContent}
        </div>
      </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {source.type && (
                      <span
                        ...
                        style={{
                          backgroundColor: `var(--color-entrenamiento-light)`,
                          color: `var(--color-entrenamiento-dark)`,
                        }}
                      >
                        {source.type}
                      </span>
