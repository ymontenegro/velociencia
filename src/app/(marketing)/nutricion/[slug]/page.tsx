...
const section = SECTIONS.nutricion;
const CONTENT_DIR = path.join(process.cwd(), "content", "nutricion");
...
export default async function NutricionArticlePage({ params }: ArticlePageProps) {
  const { slug } = await params;
  const filePath = path.join(CONTENT_DIR, `${slug}.md`);
  ...
  const relatedArticles = getAllArticles("nutricion")
  ...
  return (
    <article>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, var(--color-nutricion-dark) 0%, var(--color-nutricion) 50%, var(--color-nutricion-light) 100%)`,
          minHeight: "40vh",
        }}
      >
      ...
      <div className="mx-auto max-w-[68ch] px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        <div className={`prose prose-lg prose-nutricion`}>
          {mdxContent}
        </div>
      </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {source.type && (
                      <span
                        ...
                        style={{
                          backgroundColor: `var(--color-nutricion-light)`,
                          color: `var(--color-nutricion-dark)`,
                        }}
                      >
                        {source.type}
                      </span>
